namespace BookShop
{
    ...
    public class StartUp
    {
        ...
        public static string GetMostRecentBooks(BookShopContext context)
        {
            ...
            {
                result[i] = $"--{categories[i].CategoryName}";

                for (int j = 0; j < categories[i].Books.Length; j++)
                {
                    result[i] += Environment.NewLine + $"{categories[i].Books[j].Title} " +
                        $"({categories[i].Books[j].ReleaseDate.Value.Year})";
                }
            }
        ...
        {
            Book[] books = context.Books.Where(b => b.ReleaseDate.Value.Year < 2010)
                                        .ToArray();
