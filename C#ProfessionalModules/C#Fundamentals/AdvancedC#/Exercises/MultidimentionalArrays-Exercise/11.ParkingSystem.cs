...
{ 
    public class ParkingSystem
    {
        ...
        {
             Solution without matrices = 100100 in Judge

            string[] size = Console.ReadLine().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            int rows = int.Parse(size[0]);
            int cols = int.Parse(size[1]);

            Dictionaryint, HashSetint parking = new Dictionaryint, HashSetint();
        ...
        private static bool IsOccupied(Dictionaryint, HashSetint parking, int row, int col)
        {
            ...
            {
                parking.Add(row, new HashSetint());
            }
            return false;
        }
    }
